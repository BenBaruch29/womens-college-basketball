...
NA's weren't an issue in this section as that meant the player never transferred from another college. What was a problem, however, was a high school or something else included in that column. In the code block below, we found all of the schools listed in this column that are not universities. A lot of them are development programs, high schools or even social media handles. One cool program we found was the Lubbock Titans, an athletics program for home schooled kids in western Texas. 

To find all of these, we ran a simple group_by and summarize to see each of the previous schools listen in the data frame. Then we parsed through it, looking for any abnormalities. It's certainly possible we missed some listings, though we found a good chunk that turned out they didn't belong. 
```{r}
...
wbb_rosters23 %>% group_by(previous_school_clean) %>% summarize(total = n())
